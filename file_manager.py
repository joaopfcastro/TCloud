...
_PUBLIC_SHARE_PASSWORD_ITERATIONS = 200_000
_PUBLIC_SHARE_DEFAULT_SESSION_SECONDS = 1800


class PublicShareError(Exception):
    def __init__(self, code: str, message: str, *, status: int = 400):
        super().__init__(message)
        self.code = str(code or "public_share_error")
...
class FileManager:
    ...
    def hash_share_password(self, password: str) -> str:
        ...
        derived = hashlib.pbkdf2_hmac(
            ...
            salt,
            _PUBLIC_SHARE_PASSWORD_ITERATIONS,
        )
        return "pbkdf2_sha256${iterations}${salt}${digest}".format(
            iterations=_PUBLIC_SHARE_PASSWORD_ITERATIONS,
            salt=base64.b64encode(salt).decode("ascii"),
            digest=base64.b64encode(derived).decode("ascii"),
        )

    def verify_share_password(self, password: str, password_hash: str) -> bool:
        raw_password = str(password or "")
        encoded_hash = str(password_hash or "").strip()
        if not raw_password or not encoded_hash:
            return False
        try:
    ...
    async def _generate_unique_share_public_id(self) -> str:
        while True:
            candidate = secrets.token_urlsafe(9).rstrip("=").replace("/", "").replace("+", "")
            if not candidate:
                continue
            if not await self._db.public_id_exists(candidate):
                return candidate

    @staticmethod
    def _normalize_share_access_count(value) -> int:
        try:
    ...
    @staticmethod
    def _normalize_share_limit(value):
        if value in (None, "", False):
            return None
        try:
            limit = int(value)
        except (TypeError, ValueError):
            raise ValueError("max_access must be an integer")
        if limit <= 0:
            raise ValueError("max_access must be greater than zero")
        return limit

    def _share_is_active(self, sharing: dict | None) -> bool:
        payload = sharing if isinstance(sharing, dict) else {}
        ...
        max_access = payload.get("max_access")
        if max_access not in (None, "", False):
            try:
                if self._normalize_share_access_count(payload.get("access_count")) >= int(max_access):
                    return False
            except (TypeError, ValueError):
                return False
    ...
    @staticmethod
    def _normalize_share_inheritance_override(value) -> str:
        normalized = str(value or "").strip().lower()
        return normalized if normalized in {"inherit", "hidden", "direct"} else "inherit"

    def _share_is_hidden_from_parent(self, sharing: dict | None) -> bool:
        payload = sharing if isinstance(sharing, dict) else {}
        return self._normalize_share_inheritance_override(payload.get("inheritance_override")) == "hidden"

    async def _find_active_share_ancestor(
        self,
        normalized_path: str,
        *,
        ...
        current = (
            normalized_path
            if include_self and entry_kind == "directory"
            else self._db._parent_path(normalized_path)
        )
        while current:
            dir_doc = await self._db.get_directory_doc(current)
            sharing = dir_doc.get("sharing") if dir_doc else None
            if isinstance(sharing, dict) and self._share_is_active(sharing):
                return current, sharing
            if current == "/":
                break
            current = self._db._parent_path(current)
        return "", None

    def _serialize_sharing_state(
        self,
        sharing: dict | None,
        *,
        mode: str = "none",
        inherited_from_path: str = "",
        inherited_public_id: str = "",
        ancestor_path: str = "",
        ancestor_public_id: str = "",
    ) -> dict:
        payload = dict(sharing or {})
        public_id = str(payload.get("public_id") or "").strip()
        requires_password = bool(str(payload.get("password") or "").strip())
        inheritance_override = self._normalize_share_inheritance_override(payload.get("inheritance_override"))
        hidden_from_parent = inheritance_override == "hidden"
        expires_at = payload.get("expires_at")
        expires_iso = ""
        if isinstance(expires_at, datetime):
            normalized_expires = expires_at if expires_at.tzinfo else expires_at.replace(tzinfo=timezone.utc)
            expires_iso = normalized_expires.isoformat()

        share_mode = mode if mode in {"direct", "inherited"} else "none"
        root_public_id = inherited_public_id or public_id
        active_ancestor_public_id = ancestor_public_id or inherited_public_id
        active_ancestor_path = ancestor_path or inherited_from_path
        return {
            "is_shared": share_mode in {"direct", "inherited"},
            "mode": share_mode,
            "active": self._share_is_active(payload) if share_mode == "direct" else bool(root_public_id),
            "public_id": public_id if share_mode == "direct" else "",
            "root_public_id": root_public_id if share_mode == "inherited" else public_id,
            "inherited_from_path": inherited_from_path if share_mode == "inherited" else "",
            "inheritance_override": inheritance_override,
            "hidden_from_parent": hidden_from_parent,
            "can_hide_from_parent": bool(active_ancestor_public_id),
            "has_active_ancestor": bool(active_ancestor_public_id),
            "ancestor_public_id": active_ancestor_public_id,
            "ancestor_path": active_ancestor_path,
            "requires_password": requires_password,
            "expires_at": expires_iso,
            "max_access": payload.get("max_access"),
            "access_count": self._normalize_share_access_count(payload.get("access_count")),
        }

    async def describe_sharing_state(self, virtual_path: str, *, is_directory: bool | None = None) -> dict:
        normalized = self._db._normalize_path(virtual_path)
        entry_kind = None
        entry_doc = None
        ...
        if is_directory is True:
            ...
            entry_doc = await self._db.get_directory_doc(normalized)
        elif is_directory is False:
            entry_kind = "file"
            entry_doc = await self._db.get_file(normalized)
        else:
            entry_kind, entry_doc = await self._db.get_entry(normalized)

        if entry_doc:
            sharing = entry_doc.get("sharing")
            if isinstance(sharing, dict) and str(sharing.get("public_id") or "").strip():
                ancestor_path, ancestor_sharing = await self._find_active_share_ancestor(
                    normalized,
                    entry_kind=entry_kind,
                    include_self=False,
                )
                ancestor_public_id = str((ancestor_sharing or {}).get("public_id") or "").strip()
                return self._serialize_sharing_state(
                    sharing,
                    mode="direct",
                    ancestor_path=ancestor_path,
                    ancestor_public_id=ancestor_public_id,
                )

            if isinstance(sharing, dict) and self._share_is_hidden_from_parent(sharing):
                ancestor_path, ancestor_sharing = await self._find_active_share_ancestor(normalized, entry_kind=entry_kind)
                ancestor_public_id = str((ancestor_sharing or {}).get("public_id") or "").strip()
                return self._serialize_sharing_state(
                    sharing,
                    mode="none",
        ...
        if ancestor_sharing:
            return self._serialize_sharing_state(
                ancestor_sharing,
                mode="inherited",
                inherited_from_path=ancestor_path,
                inherited_public_id=str(ancestor_sharing.get("public_id") or "").strip(),
            )

        return self._serialize_sharing_state(None, mode="none")

    async def list_directory_with_sharing(self, virtual_path: str) -> tuple[list[dict], dict]:
        items = await self.list_directory(virtual_path)
        share_context = await self.describe_sharing_state(virtual_path, is_directory=True)
        inherited_root_public_id = ""
        inherited_from_path = ""
        inherited_requires_password = False
        inherited_expires_at = ""
        inherited_max_access = None
        inherited_access_count = 0

        if share_context.get("mode") == "direct":
            inherited_root_public_id = str(share_context.get("public_id") or "").strip()
            inherited_requires_password = bool(share_context.get("requires_password"))
            inherited_expires_at = str(share_context.get("expires_at") or "")
            inherited_max_access = share_context.get("max_access")
            inherited_access_count = self._normalize_share_access_count(share_context.get("access_count"))
        elif share_context.get("mode") == "inherited":
            inherited_root_public_id = str(share_context.get("root_public_id") or "").strip()
            inherited_from_path = str(share_context.get("inherited_from_path") or "").strip()
            inherited_requires_password = bool(share_context.get("requires_password"))
            inherited_expires_at = str(share_context.get("expires_at") or "")
            inherited_max_access = share_context.get("max_access")
            inherited_access_count = self._normalize_share_access_count(share_context.get("access_count"))
        ...
        for item in items:
            ...
            if sharing and str(sharing.get("public_id") or "").strip():
                sharing_state = self._serialize_sharing_state(sharing, mode="direct")
            elif sharing and self._share_is_hidden_from_parent(sharing):
                sharing_state = self._serialize_sharing_state(
                    sharing,
                    mode="none",
                    ancestor_path=inherited_from_path or virtual_path,
                    ancestor_public_id=inherited_root_public_id,
                )
            elif inherited_root_public_id:
                sharing_state = {
                    "is_shared": True,
                    "mode": "inherited",
                    "active": True,
                    "public_id": "",
                    "root_public_id": inherited_root_public_id,
                    "inherited_from_path": inherited_from_path or virtual_path,
                    "inheritance_override": "inherit",
                    "hidden_from_parent": False,
                    "can_hide_from_parent": True,
                    "has_active_ancestor": True,
                    "ancestor_public_id": inherited_root_public_id,
                    "ancestor_path": inherited_from_path or virtual_path,
                    "requires_password": inherited_requires_password,
                    "expires_at": inherited_expires_at,
                    "max_access": inherited_max_access,
                    "access_count": inherited_access_count,
                }
            else:
                sharing_state = self._serialize_sharing_state(None, mode="none")

            enriched = dict(item)
            enriched["sharing_state"] = sharing_state
            enriched_items.append(enriched)

        return enriched_items, {
            "current_path": self._db._normalize_path(virtual_path),
            "mode": share_context.get("mode") or "none",
            "active_ancestor_public_id": inherited_root_public_id if share_context.get("mode") == "inherited" else "",
            "active_ancestor_path": inherited_from_path if share_context.get("mode") == "inherited" else "",
            "public_id": share_context.get("public_id") or "",
    ...
    async def configure_share(
        ...
        *,
        ...
        reset_access_count: bool = False,
    ) -> tuple[str, dict]:
        normalized = self._db._normalize_path(virtual_path)
        entry_kind, entry_doc = await self._db.get_entry(normalized)
        if not entry_doc or not entry_kind:
            raise FileNotFoundError(f"Item not found: {normalized}")

        existing_sharing = dict(entry_doc.get("sharing") or {})
        existing_override = self._normalize_share_inheritance_override(existing_sharing.get("inheritance_override"))
        public_id = str(existing_sharing.get("public_id") or "").strip()
        if not public_id or regenerate_link:
            public_id = await self._generate_unique_share_public_id()

        access_count = 0 if (regenerate_link or reset_access_count) else self._normalize_share_access_count(existing_sharing.get("access_count"))
        sharing_payload = {
            "public_id": public_id,
            "access_count": access_count,
        }
        if existing_override != "inherit":
            sharing_payload["inheritance_override"] = existing_override

        if password_enabled:
            if str(password or "").strip():
                sharing_payload["password"] = self.hash_share_password(password)
            elif str(existing_sharing.get("password") or "").strip():
                sharing_payload["password"] = str(existing_sharing.get("password") or "").strip()

        normalized_limit = self._normalize_share_limit(max_access)
        if normalized_limit is not None:
            sharing_payload["max_access"] = normalized_limit

        if expires_at is not None:
            normalized_expires = expires_at if expires_at.tzinfo else expires_at.replace(tzinfo=timezone.utc)
            sharing_payload["expires_at"] = normalized_expires

        saved_kind, saved_doc = await self._db.set_sharing(normalized, sharing_payload)
        if not saved_kind or not saved_doc:
            raise FileNotFoundError(f"Item not found: {normalized}")
        return saved_kind, saved_doc

    async def remove_share(self, virtual_path: str) -> None:
        normalized = self._db._normalize_path(virtual_path)
        removed = await self._db.clear_sharing(normalized)
        if not removed:
            raise FileNotFoundError(f"Item not found: {normalized}")

    async def set_share_inheritance_override(self, virtual_path: str, hidden: bool) -> tuple[str, dict]:
        normalized = self._db._normalize_path(virtual_path)
        override = "hidden" if hidden else "inherit"
        saved_kind, saved_doc = await self._db.set_share_inheritance_override(normalized, override)
        if not saved_kind or not saved_doc:
            raise FileNotFoundError(f"Item not found: {normalized}")
        return saved_kind, saved_doc

    async def is_hidden_from_public_ancestor(self, public_root_path: str, target_path: str) -> bool:
        root = self._db._normalize_path(public_root_path)
        target = self._db._normalize_path(target_path)
        if target == root:
            return False
        prefix = root.rstrip("/") + "/"
        ...
        for segment in [part for part in relative.split("/") if part]:
            current = self._db._normalize_path(f"{current.rstrip('/')}/{segment}")
            entry_kind, entry_doc = await self._db.get_entry(current)
            if not entry_kind or not entry_doc:
                continue
            if self._share_is_hidden_from_parent(entry_doc.get("sharing")):
                return True
        return False

    async def list_public_directory(self, public_root_path: str, virtual_path: str) -> list[dict]:
        items = await self.list_directory(virtual_path)
        visible_items = []
        for item in items:
            ...
            if not item_path:
                continue
            if await self.is_hidden_from_public_ancestor(public_root_path, item_path):
                continue
            visible_items.append(item)
        return visible_items

    async def collect_public_archive_entries(self, public_root_path: str, directory_path: str) -> dict:
        normalized_root = self._db._normalize_path(public_root_path)
        normalized_directory = self._db._normalize_path(directory_path)
        if await self.is_hidden_from_public_ancestor(normalized_root, normalized_directory):
            raise PublicShareError("share_item_not_found", "Item compartilhado não encontrado.", status=404)

        directory_entries: set[str] = {""}
        file_entries: list[dict] = []

        async def visit(current_path: str, relative_prefix: str = "") -> None:
            items = await self.list_public_directory(normalized_root, current_path)
            for item in items:
                item_path = str(item.get("path") or "").strip()
                item_name = str(item.get("name") or "").strip() or Path(item_path).name
                if not item_path or not item_name:
                    continue
                archive_relative = f"{relative_prefix}/{item_name}".strip("/")
                if item.get("is_directory"):
                    directory_entries.add(archive_relative)
                    await visit(item_path, archive_relative)
                    continue

                file_meta = await self.get_file_meta(item_path)
                if not file_meta:
                    continue
                file_entries.append({
                    ...
                    "size": int(file_meta.get("size") or item.get("size") or 0),
                })

        await visit(normalized_directory)
        return {
            "files": sorted(file_entries, key=lambda entry: entry["archive_path"]),
            "directories": sorted(directory_entries),
            "total_bytes": sum(int(entry.get("size") or 0) for entry in file_entries),
            "file_count": len(file_entries),
        }

    async def collect_public_selection_entries(
        self,
        ...
        *,
        ...
        normalized_root = self._db._normalize_path(public_root_path)
        if await self.is_hidden_from_public_ancestor(normalized_root, normalized_root):
            raise PublicShareError("share_item_not_found", "Item compartilhado não encontrado.", status=404)

        normalized_relatives: list[str] = []
        seen_relatives: set[str] = set()
        for raw_relative in relative_paths or []:
            relative = self.normalize_public_share_relative_path(raw_relative)
            if relative in seen_relatives:
                continue
            normalized_relatives.append(relative)
            seen_relatives.add(relative)

        directory_entries: set[str] = set()
        file_entries: list[dict] = []
        selected_roots: list[dict] = []
        selected_directory_paths: list[str] = []
        added_file_paths: set[str] = set()
        counters = {
            "files": 0,
            "folders": 0,
            "total_items": 0,
        ...
        async def add_file(file_path: str, archive_relative: str, fallback_item: dict | None = None) -> None:
            if counters["truncated"]:
                return
            normalized_file = self._db._normalize_path(file_path)
            if normalized_file in added_file_paths:
                return
            if counters["total_items"] >= max_items:
                counters["truncated"] = True
                return
            file_meta = await self.get_file_meta(normalized_file)
            if not file_meta:
                return
            fallback = fallback_item if isinstance(fallback_item, dict) else {}
            added_file_paths.add(normalized_file)
            counters["files"] += 1
            counters["total_items"] += 1
            file_entries.append({
                "virtual_path": normalized_file,
                "file_meta": file_meta,
                "archive_path": archive_relative.strip("/") or Path(normalized_file).name,
                "size": int(file_meta.get("size") or fallback.get("size") or 0),
            })

        async def visit_directory(current_path: str, relative_prefix: str) -> None:
            if counters["truncated"]:
                return
            normalized_current = self._db._normalize_path(current_path)
            counters["directory_visits"] += 1
            if counters["directory_visits"] > max_directories:
                counters["truncated"] = True
                return

            items = await self.list_public_directory(normalized_root, normalized_current)
            for item in items:
                if counters["truncated"]:
                    return
                item_path = str(item.get("path") or "").strip()
                ...
                if item.get("is_directory"):
                    ...
                    counters["total_items"] += 1
                    directory_entries.add(archive_relative)
                    await visit_directory(item_path, archive_relative)
                    continue
                await add_file(item_path, archive_relative, item)

        candidates: list[dict] = []
        for relative in normalized_relatives:
            full_path = normalized_root if not relative else self._db._normalize_path(f"{normalized_root}/{relative}")
            entry_kind, entry_doc = await self._db.get_entry(full_path)
            if not entry_kind or not entry_doc:
                raise PublicShareError("share_item_not_found", "Item compartilhado não encontrado.", status=404)
            if await self.is_hidden_from_public_ancestor(normalized_root, full_path):
                raise PublicShareError("share_item_not_found", "Item compartilhado não encontrado.", status=404)
            candidates.append({
        ...
        candidates.sort(key=lambda item: (item["relative_path"].count("/"), item["relative_path"]))
        for candidate in candidates:
            candidate_path = candidate["path"]
            if any(
                candidate_path != selected_dir and candidate_path.startswith(selected_dir.rstrip("/") + "/")
                for selected_dir in selected_directory_paths
            ):
                counters["skipped_nested_count"] += 1
                continue
            ...
            if candidate["kind"] == "directory":
                selected_directory_paths.append(candidate_path)
                if counters["total_items"] >= max_items:
                    counters["truncated"] = True
                    continue
                counters["folders"] += 1
                ...
                await visit_directory(candidate_path, relative)
                continue

            doc = candidate["doc"] if isinstance(candidate["doc"], dict) else {}
            archive_relative = relative or str(doc.get("filename") or Path(candidate_path).name)
            await add_file(candidate_path, archive_relative, doc)

        sorted_files = sorted(file_entries, key=lambda entry: entry["archive_path"])
        return {
            "files": sorted_files,
            "directories": sorted(directory_entries),
            "total_bytes": sum(int(entry.get("size") or 0) for entry in sorted_files),
            "file_count": len(sorted_files),
            "folder_count": int(counters["folders"]),
            "total_items": int(counters["files"] + counters["folders"]),
            "selected_count": len(normalized_relatives),
            "selected_roots": selected_roots,
            "skipped_nested_count": int(counters["skipped_nested_count"]),
            "truncated": bool(counters["truncated"]),
        }

    async def collect_public_directory_metrics(
        self,
        public_root_path: str,
        directory_path: str,
        *,
        ...
        async def visit(current_path: str) -> None:
            ...
            if metrics["directory_count"] > max_directories:
                metrics["truncated"] = True
                return

            items = await self.list_public_directory(normalized_root, normalized_current)
            for item in items:
                if metrics["total_items"] >= max_items:
                    metrics["truncated"] = True
                    return
                ...
                if item.get("is_directory"):
                    metrics["folders"] += 1
                    if item_path:
                        await visit(item_path)
                    continue

                metrics["files"] += 1
                try:
                    metrics["total_bytes"] += int(item.get("size") or 0)
                except (TypeError, ValueError):
                    continue
    ...
    async def get_shared_target(self, public_id: str) -> tuple[str, dict]:
        target_kind, target_doc = await self._db.find_shared_target(public_id)
        if not target_doc or not target_kind:
            raise PublicShareError("share_not_found", "Link público não encontrado.", status=404)
        return target_kind, target_doc

    async def grant_public_share_access(self, public_id: str, password: str = "") -> tuple[str, dict]:
        target_kind, target_doc = await self.get_shared_target(public_id)
        sharing = dict(target_doc.get("sharing") or {})
        if not self._share_is_active(sharing):
            ...
            raise PublicShareError("share_unavailable", "Este link público não está disponível.", status=410)

        password_hash = str(sharing.get("password") or "").strip()
        if password_hash and not self.verify_share_password(password, password_hash):
            raise PublicShareError("share_password_invalid", "Senha incorreta.", status=403)

        claimed_kind, claimed_doc = await self._db.claim_share_access(public_id, datetime.now(timezone.utc))
        if not claimed_kind or not claimed_doc:
            refreshed_kind, refreshed_doc = await self._db.find_shared_target(public_id)
            refreshed_sharing = dict((refreshed_doc or {}).get("sharing") or {})
            expires_at = refreshed_sharing.get("expires_at")
            if isinstance(expires_at, datetime):
                normalized_expires = expires_at if expires_at.tzinfo else expires_at.replace(tzinfo=timezone.utc)
                if normalized_expires <= datetime.now(timezone.utc):
                    raise PublicShareError("share_expired", "Este link público expirou.", status=410)
            max_access = refreshed_sharing.get("max_access")
            if max_access not in (None, "", False):
                raise PublicShareError("share_access_exhausted", "Este link público atingiu o limite de acessos.", status=410)
            raise PublicShareError("share_unavailable", "Este link público não está disponível.", status=410)

        return claimed_kind, claimed_doc

    def normalize_public_share_relative_path(self, relative_path: str | None) -> str:
        raw = str(relative_path or "").strip().replace("\\", "/")
        if not raw or raw == "/":
            return ""
        parts = [segment for segment in raw.split("/") if segment not in ("", ".")]
        if any(segment == ".." for segment in parts):
            raise PublicShareError("share_invalid_path", "Caminho compartilhado inválido.", status=400)
        return "/".join(parts)

    async def resolve_public_share_path(
        self,
        public_root_path: str,
        ...
        *,
        ...
        normalized_root = self._db._normalize_path(public_root_path)
        relative = self.normalize_public_share_relative_path(relative_path)
        full_path = normalized_root if not relative else self._db._normalize_path(f"{normalized_root}/{relative}")

        if expect_directory is True:
            if not await self._db.directory_exists(full_path):
                raise PublicShareError("share_directory_not_found", "Pasta compartilhada não encontrada.", status=404)
        elif expect_directory is False:
            if not await self._db.file_exists(full_path):
        ...
        if enforce_visibility and await self.is_hidden_from_public_ancestor(normalized_root, full_path):
            raise PublicShareError("share_item_not_found", "Item compartilhado não encontrado.", status=404)
    ...
    async def list_trash(self, *, include_counts: bool = False) -> list[dict]:
        ...
        counts_by_root = await self._db.count_trash_descendants_by_root_ids(root_ids)
        items = []
        for root in roots:
            root_id = str(root.get("trash_root_entry_id") or root.get("trash_entry_id") or "").strip()
            counts = counts_by_root.get(root_id, {})
            item = dict(root)
