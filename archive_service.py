...
class ArchiveService:
    ...
    async def create_extract_job(
        ...
        *,
        ...
        normalized_destination = await self._assert_existing_directory_destination(destination)
        job = self._new_job(
            job_type="extract",
    ...
    async def _resolve_extract_destination(self, job: dict, archive_name: str) -> str:
        ...
        destination = await self._assert_existing_directory_destination(job.get("destination"))
    ...
    async def _resolve_output_archive_path(self, *, destination_dir: str, archive_name: str, overwrite_mode: str) -> str:
        destination_dir = await self._assert_existing_directory_destination(destination_dir)
    ...
    async def _assert_existing_directory_destination(self, path: str) -> str:
        if not str(path or "").strip():
            raise ArchiveValidationError("O destino precisa ser uma pasta existente.")
        normalized = _normalize_virtual_path(path)
        if normalized == "/":
            return normalized
        if await self._file_manager.is_directory(normalized):
            return normalized
        if await self._file_manager.is_file(normalized):
            raise ArchiveValidationError("O destino precisa ser uma pasta, mas o caminho aponta para um arquivo.")
        raise ArchiveValidationError("O destino precisa ser uma pasta existente.")
