(function () {
    ...
    function getWindowRoot() {
        let root = document.getElementById('tcloud-window-layer');
        if (root) return root;
        root = document.createElement('div');
        root.id = 'tcloud-window-layer';
        root.className = 'tcloud-window-layer';
    ...
    function ensureWindowInRoot(record) {
        if (!record?.element) return null;
        const root = getWindowRoot();
        if (!root || record.element.parentElement === root) return root;
        root.appendChild(record.element);
        if (!record.didRepairWindowRoot) {
            record.didRepairWindowRoot = true;
            console.warn('WindowManager repaired parent for', record.id);
        }
        return root;
    }
    ...
    function getFocusableWindows() {
        return Array.from(state.windows.values())
            .filter((record) => record && record.status !== 'closed' && !record.minimized)
            .sort((a, b) => Number(b.zIndex || 0) - Number(a.zIndex || 0));
    }

    function closeActiveWindow() {
        const active = state.activeId ? state.windows.get(state.activeId) : getFocusableWindows()[0];
        if (!active || active.status === 'closed') return false;
        close(active.id);
        const next = getFocusableWindows()[0];
        if (next) focus(next.id);
        return true;
    }

    function cycleWindows(direction = 1) {
        const windows = getFocusableWindows().sort((a, b) => Number(a.zIndex || 0) - Number(b.zIndex || 0));
        if (windows.length <= 1) return false;
        const activeIndex = Math.max(0, windows.findIndex((record) => record.id === state.activeId));
        const step = direction < 0 ? -1 : 1;
        const nextIndex = (activeIndex + step + windows.length) % windows.length;
        focus(windows[nextIndex].id);
        return true;
    }
    ...
    function getResizeDirFromPoint(record, x, y, edgeSize = 9) {
        if (!record || record.status === 'maximized' || record.minimized || record.status === 'closed') return '';
        ...
        const nearBottom = y >= rect.bottom - edgeSize;
        const nearLeft = x <= rect.left + edgeSize;
        const nearRight = x >= rect.right - edgeSize;
    ...
    function getTopWindowAtPoint(x, y) {
        return Array.from(state.windows.values())
            .filter((record) => {
                if (!record || record.status === 'closed' || record.minimized) return false;
                const rect = record.element.getBoundingClientRect();
    ...
    function wireGlobalResizeHoverTracking() {
        document.addEventListener('pointermove', (event) => {
            if (event.buttons || document.querySelector('.tcloud-window.is-resizing, .tcloud-window.is-dragging')) return;
            const target = event.target;
            if (target?.closest?.('[data-window-action], button, a, input, textarea, select, [contenteditable="true"], .tcloud-window-layout-menu, .tcloud-window-dock')) {
                clearAllResizeHover();
                return;
            }
            const record = getTopWindowAtPoint(event.clientX, event.clientY);
            const dir = getResizeDirFromPoint(record, event.clientX, event.clientY);
            if (!record || !dir) {
                clearAllResizeHover();
                return;
            }
            state.windows.forEach((item) => {
                if (item !== record) clearResizeHover(item);
            });
    ...
    function wireIframeContentFocusBridge(record, iframe) {
        ...
        frameDocument.addEventListener('keydown', (event) => handleWindowShortcut(event, { record }), true);
    }

    function handleWindowShortcut(event, options = {}) {
        if (!event || event.defaultPrevented) return false;
        const key = String(event.key || '').toLowerCase();
        const hasCommandModifier = event.metaKey || event.ctrlKey;
        const record = options.record || (state.activeId ? state.windows.get(state.activeId) : null);
        ...
        if (event.metaKey && !event.altKey && key === 'tab') {
            const handled = cycleWindows(event.shiftKey ? -1 : 1);
    ...
    function wireGlobalWindowShortcuts() {
        document.addEventListener('keydown', handleWindowShortcut, true);
    }
    ...
    function wireWindow(record) {
        ...
        if (titlebar) {
            titlebar.addEventListener('pointerdown', (event) => {
                ...
                const startStatus = record.status;
                const startSnapSide = record.snapSide;
                const startRestoreRect = cloneRect(record.restoreRect || record.rect);
                const start = {
                ...
                const beginDrag = (moveEvent) => {
                    start.dragging = true;
                    if (startStatus === 'maximized') {
                        const restoreRect = getSafeRestoreRect({
                            ...record,
                            restoreRect: startRestoreRect,
                            initialRect: record.initialRect,
                        });
                        const pointerRatio = startRect.width > 0
                            ? clamp(start.pointerX / startRect.width, 0.15, 0.85)
                            : 0.5;
                        restoreRect.x = clamp(
                            moveEvent.clientX - (restoreRect.width * pointerRatio),
                            -restoreRect.width + DEFAULTS.visibleMargin,
                            getViewportRect().width - DEFAULTS.visibleMargin,
                        );
                        restoreRect.y = clamp(
                            moveEvent.clientY - Math.min(DEFAULTS.titlebarHeight / 2, start.pointerY - startRect.y),
                            0,
                            getViewportRect().height - DEFAULTS.titlebarHeight,
                        );
                        record.rect = normalizeRect(record, restoreRect);
                    }
                    record.status = 'normal';
                    record.snapSide = null;
                    record.minimized = false;
                    record.restoreStatus = startStatus === 'maximized' || startStatus === 'snapped' ? startStatus : null;
                    record.restoreSnapSide = startSnapSide || null;
                    record.element.classList.add('is-dragging');
                    record.element.classList.remove('is-window-transitioning');
                    setIframeShield(record, true);
                    start.rect = cloneRect(record.rect);
                    start.pointerX = moveEvent.clientX;
                    start.pointerY = moveEvent.clientY;
                };

                const onMove = (moveEvent) => {
                    if (moveEvent.pointerId !== start.pointerId) return;
                    const movementX = moveEvent.clientX - start.pointerX;
                    const movementY = moveEvent.clientY - start.pointerY;
                    if (!start.dragging) {
                        const distance = Math.hypot(movementX, movementY);
                        if (distance < DEFAULTS.dragThreshold) return;
                        beginDrag(moveEvent);
                        return;
                    }
                    const viewport = getViewportRect();
    ...
    function scheduleShowLayoutMenu(record, button, delay = 520) {
        if (state.layoutMenuOpenTimer) window.clearTimeout(state.layoutMenuOpenTimer);
        const hoverToken = ++state.layoutMenuHoverToken;
        state.layoutMenuOpenTimer = window.setTimeout(() => {
            state.layoutMenuOpenTimer = 0;
            if (hoverToken !== state.layoutMenuHoverToken) return;
    ...
    function wireLayoutMenuTrigger(record, button) {
        ...
        button.addEventListener('pointerenter', (event) => {
            if (event.pointerType === 'mouse') scheduleShowLayoutMenu(record, button, 620);
        });
        button.addEventListener('pointerleave', () => scheduleHideLayoutMenu(80));
        button.addEventListener('pointerdown', () => hideLayoutMenu());
        button.addEventListener('focus', () => scheduleShowLayoutMenu(record, button, 700));
    ...
    function applyLayout(id, layout, reason = 'external-layout') {
        const record = state.windows.get(id);
        if (!record || record.status === 'closed') return false;
        return applyPersistedLayout(record, layout, reason);
    }
