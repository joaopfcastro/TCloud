...
"""
    ...
    async def _handle_api_shared_item(self, request):
        ...
        if target_kind == "directory":
            try:
                ...
                metrics = await self._file_manager.collect_public_directory_metrics(root_path, root_path)
                summary["total_size_bytes"] = int(metrics.get("total_bytes") or 0)
                summary["total_size_truncated"] = bool(metrics.get("truncated"))
            except Exception as exc:
                logger.warning("Failed to compute public share total size for %s: %s", public_id, exc, exc_info=True)
                summary["total_size_bytes"] = 0
                summary["total_size_truncated"] = True
    ...
    async def _handle_api_shared_item_browse(self, request):
        ...
        try:
            ...
            return web.json_response({
                ...
                "root_name": (
                    str(target_doc.get("path") or "/").rstrip("/").rsplit("/", 1)[-1] or "Compartilhado"
                ),
    ...
    async def _handle_api_shared_media_tracks(self, request):
        ...
        try:
            ...
            logger.info(
                "🎬 Public media tracks probe result: public_id=%s path=%s audio=%s subtitle=%s elapsed_ms=%s",
                public_id,
                target_path,
                len(audio_tracks),
                len(subtitle_tracks),
            ...
            return web.json_response({"audio": audio_tracks, "subtitle": subtitle_tracks})
        except PublicShareError as exc:
            return web.json_response({"error": exc.message, "code": exc.code}, status=exc.status)
        except asyncio.TimeoutError:
            return web.json_response({"audio": [], "subtitle": []})
    ...
    async def _handle_api_shared_web_playback_session_create(self, request):
        ...
        try:
            resolved_cloud_request = await self._resolve_cloud_file_request(target_path)
            if not resolved_cloud_request.get("file_meta"):
                return web.json_response({"error": "Arquivo compartilhado não encontrado"}, status=404)
            effective_path = str(
                resolved_cloud_request.get("resolved_path")
                or resolved_cloud_request.get("normalized_path")
                or target_path
        ...
        media_playlist_name = session.get("media_playlist_name") or "media.m3u8"
        logger.info(
            "🎬 Public share playback session ready: public_id=%s session=%s target=%s",
            public_id,
            session["id"],
            effective_path,
        )
    ...
    def _build_public_web_playback_master_playlist(self, session: dict, request) -> str:
        ...
        return "\n".join([
            ...
            "#EXT-X-VERSION:7",
            "#EXT-X-INDEPENDENT-SEGMENTS",
            "#EXT-X-STREAM-INF:BANDWIDTH=3500000,AVERAGE-BANDWIDTH=2200000",
            media_playlist_ref,
            "",
        ])
