        ...
        async with build_lock:
            ...
            try:
                if is_local:
                    ...
                    is_cached = False
                    try:
                        is_cached = bool(self._file_manager.is_file_cached(file_meta, raw_path))
                    except Exception as exc:
                        logger.debug(f"Could not check cache status for stable audio fallback {raw_path}: {exc}")
                    if not is_cached:
                        logger.info(f"🎧 Stable audio fallback skipped until source is cached: {raw_path}")
                        return None, filename or None
                    ...
                    if cached_source_path:
                        ...
                        logger.info(f"🎧 Stable audio fallback materializing already cached source: {raw_path}")
