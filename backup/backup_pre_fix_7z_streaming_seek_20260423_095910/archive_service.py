...
class ArchiveService:
    ...
    def capabilities_payload(self) -> dict:
        ...
        streaming_extract_formats = sorted(
            fmt for fmt in self._streaming_extract_formats if self._streaming_extract_enabled and streaming_extract_backend
        )
        extract_caps = {
            "zip": True,
            "tar.gz": True,
    ...
    def _is_streaming_extract_enabled_for_format(self, archive_format: str) -> bool:
        return (
            self._streaming_extract_enabled
            and bool(self._streaming_extract_backend_path())
            and archive_format in self._streaming_extract_formats
        )
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="extract_", dir=str(self._runtime_dir)) as temp_dir:
                ...
                summary = {
                    ...
                    "streaming": self._is_streaming_extract_enabled_for_format(archive_format),
                    "extract_backend": "bsdtar" if self._is_streaming_extract_enabled_for_format(archive_format) else "legacy",
                }
