...
from file_manager import scan_image_metadata, scan_pdf_metadata
...
class TCloudHTTPServer:
    ...
    async def _handle_api_files(self, request):
        ...
        try:
            ...
            return web.json_response({"items": result, "path": path})
        ...
        try:
            ...
            for item in items:
                result.append({
                    ...
                    "mime_type": item.get("meta", {}).get("mime_type") or mimetypes.guess_type(item.get("filename") or item.get("name") or str(Path(item.get("path", "")).name))[0],
                })
    ...
    async def _handle_api_torrent_info(self, request):
        ...
        try:
            ...
            if not magnet:
                return web.json_response({"error": "Magnet link ausente"}, status=400)
                
            info = await self._torrent_manager.get_info(magnet)
...
"""
    ...
    async def _resolve_cloud_file_request(self, raw_path: str | None) -> dict:
    ...
    async def _wait_for_web_hls_session_ready(self, session_id: str, timeout_seconds: float = 45.0) -> dict:
        ...
        if session:
            ...
            logger.warning(
                "🎬 Web playback HLS readiness timeout: session=%s path=%s ss=%.3f audio=%s input_mode=%s used_re=%s playlist_kind=%s elapsed_ms=%s playlist_seen=%s first_segment_seen=%s stderr_tail=%s",
                session_id,
        ...
        *,
        ...
        if is_local:
            ...
            if self._file_manager._cache.is_fully_cached(raw_path, chunks):
                ...
                stdin_mode = asyncio.subprocess.DEVNULL
                input_mode = "cached-http-range"
                logger.info("📺 Using cached raw HTTP input for HLS session: %s", raw_path)
            else:
                if start_seconds > 0:
                    internal_raw_url = self._build_internal_media_probe_url(raw_path, raw_mode=True)
                    input_args.extend([
                    ...
                    stdin_mode = asyncio.subprocess.DEVNULL
                    input_mode = "cloud-http-range"
                    logger.info("🎬 Using seekable raw HTTP input for cloud HLS session: %s", raw_path)
                else:
                    input_args.extend([
                        "-analyzeduration", "20000000",
                        "-probesize", "20000000",
        ...
        # Smart codec detection: use -c:v copy (remux) when source is H.264,
        # otherwise re-encode with libx264. Remux is ~100x faster.
        source_video_codec = None
        try:
            probe_input = None
            if is_local:
                probe_input = str(source["file_path"])
            else:
                file_meta = source.get("file_meta") or {}
                chunks = file_meta.get("chunks", [])
                if self._file_manager._cache.is_fully_cached(raw_path, chunks):
                    probe_input = self._build_internal_media_probe_url(raw_path, raw_mode=True)
            ...
            if probe_input:
                ...
                source_video_codec = (codec_out.decode().strip().lower() if codec_out else "") or None
        except Exception as exc:
            logger.debug("HLS codec probe failed for %s: %s", raw_path, exc)

        use_video_copy = source_video_codec in ("h264", "h264_qsv", "h264_nvenc")
        logger.info(
            "🎬 HLS codec decision: source=%s → %s",
            source_video_codec or "unknown",
            "remux (copy)" if use_video_copy else "transcode (libx264)",
        )

        if use_video_copy:
            # Remux: copy H.264 video stream as-is (ultra-fast, ~2-3 sec for first segment)
            video_codec_args = ["-c:v", "copy"]
        else:
            # Transcode: re-encode to H.264 (slower, ~15-30 sec for first segment)
            video_codec_args = [
                "-c:v", "libx264",
                "-preset", "ultrafast",
                "-tune", "zerolatency",
        ...
        # Keyframe control only applies when encoding video (not copy)
        ...
        session = {
            ...
            "temp_concat_path": temp_concat_path,
            "process": process,
            "stderr_tail": [],
            "feed_task": None,
            "stderr_task": None,
        ...
        if stdin_mode == asyncio.subprocess.PIPE:
            feed_task = asyncio.create_task(feed_ffmpeg_from_cloud())
            session["feed_task"] = feed_task
        ...
        try:
            ready_session = await self._wait_for_web_hls_session_ready(session_id)
            ...
            logger.info(
                "🎬 Web playback HLS session ready: %s (%s, audio=%s, ss=%.3f, segment_type=%s, playlist_kind=%s, input_mode=%s, used_re=%s, startup_ms=%s, playlist_ms=%s, first_segment_ms=%s)",
                session_id,
                raw_path,
                audio_index,
            ...
            if self._web_playback_hls_subtitles_mode != "off":
                ...
                bootstrap_reason = str(ready_session.get("bootstrap_reason") or "initial-open").strip().lower() or "initial-open"
                defer_subtitle_prewarm = bootstrap_reason in {
                    "server-seek",
                    "progress-control",
                    "audio-switch",
                    "audio-track-change",
            ...
            await self._destroy_web_playback_session(session_id, reason="startup-failed")
            raise
    ...
    async def _handle_api_web_playback_session_create(self, request):
        ...
        if not is_local:
            resolved_cloud_request = await self._resolve_cloud_file_request(raw_path)
            effective_path = str(
            ...
            if not resolved_cloud_request.get("file_meta"):
                logger.warning(
                    "🎬 Web playback session path unresolved: requested=%s normalized=%s decoded=%s attempts=%s",
                    raw_path,
        ...
        try:
            ...
            return web.json_response({"error": "Timeout preparando stream HLS"}, status=504)
        except Exception as exc:
        ...
        async with build_lock:
            ...
            try:
                if is_local:
                    ...
                    if not file_meta:
                        return None, filename or None
                    logger.info(f"🎧 Stable audio fallback requires full cache first: {raw_path}")
                    await self._file_manager.cache_file(raw_path)
                    temp_source_path = variant_path.with_name(f"{variant_path.stem}.{os.getpid()}.source{source_suffix}")
                    with open(temp_source_path, "wb") as source_fp:
                        async for data in self._file_manager.stream_file_range(raw_path, 0, file_meta["size"] - 1):
                            source_fp.write(data)
    ...
    async def _handle_stream(self, request):
        ...
        if is_local:
             ...
             try:
                 ...
                 transcode_required = filename.lower().endswith(('.mkv', '.avi', '.flv', '.wmv')) or (
                     (audio_selection_requested or audio_only_requested) and not suffix in STABLE_AUDIO_VARIANT_EXTENSIONS
                 )
        ...
        use_stable_audio_variant = audio_only_requested and suffix in STABLE_AUDIO_VARIANT_EXTENSIONS
        transcode_required = filename.lower().endswith(('.mkv', '.avi', '.flv', '.wmv')) or (
            (audio_selection_requested or audio_only_requested) and suffix not in STABLE_AUDIO_VARIANT_EXTENSIONS
        )
        
        # Determine if it's an image or direct-stream audio
        is_direct_stream = filename.lower().endswith(('.jpg', '.jpeg', '.png', '.gif', '.webp', '.heic', '.bmp', '.mp3', '.m4a', '.wav', '.aac', '.flac', '.ogg', '.wma', '.mp4', '.webm')) or Path(filename).suffix.lower() in SUBTITLE_EXTENSIONS

        # Determine if this is a video file (direct stream, but still worth caching for repeated plays)
        is_video = filename.lower().endswith(('.mp4', '.webm'))

        # Auto-trigger background caching for video files only
        # Videos stream directly from Telegram, but caching in background means
        # subsequent plays/seeks will be instant from disk
        ...
        if transcode_required:
            if audio_selection_requested or audio_only_requested:
                logger.info(f"🎧 audio switch path: transcoded-fallback ({raw_path}, audio={audio_track_index})")
            return await self._stream_transcoded_file(request, raw_path, is_local=False, file_meta=file_meta, force_audio_only=audio_only_requested)
        ...
        if range_header:
            # Validate If-Range (ETag must match for partial content)
            if_range = request.headers.get("If-Range")
            if if_range and if_range != etag:
                # ETag mismatch — serve full content
                range_header = None
            else:
                try:
                    range_spec = range_header.replace("bytes=", "")
        ...
        # Ensure filename is safe (handle quotes)
        safe_filename = filename.replace('"', '\\"')
        
        # Use attachment disposition for downloads, inline for streaming/playback
        ...
        headers = {
            ...
            "Content-Length": str(content_length), # Changed 'length' to 'content_length'
        ...
        # Fast path: single-chunk file fully cached → stream from disk
        # (We use StreamResponse instead of FileResponse to keep full header
        #  control — FileResponse overrides Content-Disposition with the cache
        #  file name, causing UUID filenames in the browser.)
        cached_path = self._file_manager.get_cached_file_path(file_meta, raw_path)
        if cached_path:
            ...
            with open(cached_path, "rb") as f:
                ...
                while bytes_remaining > 0:
                    data = f.read(min(262144, bytes_remaining))  # 256 KB
                    if not data:
                        break
                    await response.write(data)
        ...
        # Stream from cache (fetches from Telegram on miss) OR direct for images
        response = web.StreamResponse(status=status, headers=headers)
        await response.prepare(request)
        ...
        try:
            ...
            logger.error(f"Stream error for {raw_path}: {e}")

        await response.write_eof()

        # After a download completes, evict the temporary cache to save disk space
        # (but only if this was a download request, not a regular stream/playback)
        if is_download:
            try:
                await self._file_manager.evict_file_cache(raw_path)
            except Exception as e:
                logger.warning(f"Cache eviction after download failed: {e}")
        ...
        try:
            ...
            for item in items:
                ...
                result.append({
                    ...
                    "size": item["size"],
                    "meta": item.get("meta", {}),
                    "created_at": item.get("created_at", "").isoformat() if hasattr(item.get("created_at"), "isoformat") else "",
                    "modified_at": item.get("modified_at", "").isoformat() if hasattr(item.get("modified_at"), "isoformat") else "",
                    "is_offline": item.get("is_offline", False),
                    "is_favorite": True,
                    "is_cached": is_cached,
                    "mime_type": item.get("meta", {}).get("mime_type") or mimetypes.guess_type(item["name"])[0],
