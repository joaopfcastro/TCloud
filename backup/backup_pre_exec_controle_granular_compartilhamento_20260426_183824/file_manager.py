    ...
    async def describe_sharing_state(self, virtual_path: str, *, is_directory: bool | None = None) -> dict:
        ...
        if entry_doc:
            ...
            if isinstance(sharing, dict) and str(sharing.get("public_id") or "").strip():
                return self._serialize_sharing_state(sharing, mode="direct")

        current = normalized if entry_kind == "directory" else self._db._parent_path(normalized)
        while current:
            ...
            if isinstance(sharing, dict) and self._share_is_active(sharing):
                return self._serialize_sharing_state(
                    sharing,
                    mode="inherited",
                    inherited_from_path=current,
                    inherited_public_id=str(sharing.get("public_id") or "").strip(),
                )
            if current == "/":
                break
