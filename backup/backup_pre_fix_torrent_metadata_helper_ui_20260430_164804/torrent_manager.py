...
class TorrentManager:
    ...
    async def get_info(self, magnet_or_file: str) -> Dict[str, Any]:
        ...
        if process.returncode != 0:
            payload = self._extract_node_json(stderr_text) or self._extract_node_json(stdout_text) or {}
            error_msg = payload.get("error") or stderr_text or stdout_text or "Failed to get torrent metadata"
