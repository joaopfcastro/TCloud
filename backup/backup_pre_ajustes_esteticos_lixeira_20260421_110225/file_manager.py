...
class FileManager:
    ...
    async def _refresh_generated_video_metadata(self, video_path: str) -> None:
        ...
        await self._db._files.update_one(
            ...
            {"$set": {"meta": meta, "modified_at": datetime.now(timezone.utc)}},
        )
    ...
    async def trash_file(self, virtual_path: str) -> dict:
        ...
        if self._is_generated_sidecar_doc(file_doc):
            ...
            if parent_video_path:
                await self._refresh_generated_video_metadata(parent_video_path)
    ...
    async def restore_trash(self, entry_ids: list[str] | None = None, *, restore_all: bool = False) -> dict:
        ...
        if restore_all:
            root_ids = [item.get("trash_root_entry_id") for item in await self.list_trash()]
            root_ids = [str(entry_id).strip() for entry_id in root_ids if str(entry_id).strip()]

        restored = []
        errors = []

        for root_id in root_ids:
            tree = await self._db.get_trash_tree(root_id)
            ...
            try:
                if root_dir:
                    ...
                    for video_path in restored_video_paths:
                        await self._refresh_generated_video_metadata(video_path)
                ...
                if self._is_externalized_video_doc(restored_root_doc):
                    await self._refresh_generated_video_metadata(restored_root_path)
