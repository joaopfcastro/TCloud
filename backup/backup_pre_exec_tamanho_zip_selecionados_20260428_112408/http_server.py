        ...
        try:
            ...
            response = web.StreamResponse(
                ...
                headers={
                    ...
                    "X-Archive-Root": current_relative_path,
                },
            )
                ...
                try:
                    with zipfile.ZipFile(writer, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=6) as archive:
                        ...
                        for entry in manifest.get("files") or []:
                            archive_path = f"{archive_root}/{str(entry.get('archive_path') or '').strip('/')}"
                    ...
                    logger.error("Public ZIP generation failed for %s/%s: %s", public_id, relative_path, exc, exc_info=True)
                    writer.abort(exc)
                finally:
                    writer.close()
