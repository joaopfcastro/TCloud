...
"""
    ...
    async def _handle_api_shared_item_access(self, request):
        ...
        try:
            ...
            session_expires_at = datetime.now(timezone.utc) + timedelta(
                seconds=max(60, int(getattr(Config, "PUBLIC_SHARE_SESSION_TTL_SECONDS", 1800)))
            )
    ...
    async def _handle_api_recents(self, request):
        try:
            ...
            items = await self._file_manager.get_recents(limit)
            result = []
            for item in items:
                serialized_item = await self._serialize_browser_item(request, {
                    **item,
    ...
    async def _dispatch_runtime_function(self, app: dict, function_id: str, payload: dict, session_payload: dict):
        ...
        if function_id == "recents.list":
            try:
                recents_limit = int(payload.get("limit") or 50)
            except (TypeError, ValueError):
                recents_limit = 50
            recents_limit = max(1, min(recents_limit, 200))
            items = await self._file_manager.get_recents(recents_limit)
            return {"items": self._normalize_file_listing_items(items)}
