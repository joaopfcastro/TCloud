...
const PDF_TOOLS_OPENED_RECENTS_KEY = "pdf-tools.openedRecents.v1";
...
function saveRecentPdfs() {
  try {
    localStorage.setItem(PDF_TOOLS_OPENED_RECENTS_KEY, JSON.stringify(app.recentPdfs.slice(0, 12)));
  } catch (error) {
    console.warn("Nao foi possivel salvar historico de PDFs abertos", error);
  }
}

async function loadRecentPdfs() {
  try {
    const stored = JSON.parse(localStorage.getItem(PDF_TOOLS_OPENED_RECENTS_KEY) || "[]");
    app.recentPdfs = mergeRecentPdfs([...(Array.isArray(stored) ? stored : []), ...app.tabs]);
  } catch (error) {
    console.warn("Nao foi possivel carregar historico de PDFs abertos", error);
...
function recordRecentPdfOpen(item) {
  app.recentPdfs = mergeRecentPdfs([
    {
      ...item,
