...
class ArchiveService:
    ...
    def capabilities_payload(self) -> dict:
        ...
        return {
            ...
            "extract": {
                "zip": True,
                "tar.gz": True,
                "7z": py7zr is not None,
                "rar": bool(rarfile is not None and rar_backend),
            },
            "compress": {
                "zip": True,
                "tar.gz": True,
    ...
    def _friendly_error_message_for_code(self, code: str, default_message: str) -> str:
        return {
            ...
            "archive_stream_pipeline_failed": "Falha ao compactar: erro interno no pipeline de streaming.",
            "archive_upload_failed": "Falha ao compactar: não foi possível enviar o arquivo final.",
    ...
    def _classify_job_error(self, exc: Exception | str, job: dict) -> tuple[str, str]:
        ...
        if archive_format in {"zip", "tar.gz"} and self._is_streaming_enabled_for_format(archive_format):
            return "archive_stream_pipeline_failed", self._friendly_error_message_for_code("archive_stream_pipeline_failed", message)
        return "archive_failed", message

    def _is_streaming_enabled_for_format(self, archive_format: str) -> bool:
    ...
    async def _run_extract_job(self, job: dict, *, password: str) -> None:
        try:
            archive_meta = await self._file_manager.get_file_meta(job["archive_path"])
            if not archive_meta:
            ...
            with tempfile.TemporaryDirectory(prefix="extract_", dir=str(self._runtime_dir)) as temp_dir:
                ...
                self._touch_job(
                    ...
                    unit="items",
                    detail="Materializando origem para extração",
                )
                ...
                uploaded_files = await self._upload_extracted_tree(job, extract_root, destination_root, job.get("overwrite_mode") or self._default_overwrite_mode)
                summary = {
                    "uploaded_files": uploaded_files,
                    "destination_root": destination_root,
                    "warnings": len(job.get("warnings") or []),
                }
                self._mark_job_done(job, message="Extração concluída.", summary=summary, output_path=destination_root)
    ...
    async def _run_compress_job(self, job: dict) -> None:
        try:
            ...
            if self._is_streaming_enabled_for_format(archive_format):
                ...
                with tempfile.TemporaryDirectory(prefix="compress_", dir=str(self._runtime_dir)) as temp_dir:
                    ...
                    self._touch_job(
                        ...
                        current_file=archive_name,
                        unit="items",
                        detail=f"Upload final de {archive_name}",
                    )
                    await self._file_manager.upload_file(output_file, target_path)
                    self._touch_job(
                        job,
                        stage="uploading",
                        current=1,
    ...
    def _assert_extract_supported(self, archive_format: str) -> None:
        caps = self.capabilities_payload()["extract"]
        if not caps.get(archive_format):
            raise ArchiveCapabilityError(f"Extração de {archive_format} indisponível neste ambiente.")

    def _assert_compress_supported(self, archive_format: str) -> None:
        caps = self.capabilities_payload()["compress"]
        if not caps.get(archive_format):
            raise ArchiveCapabilityError(f"Compactação em {archive_format} indisponível neste ambiente.")
    ...
    async def _collect_archive_entries(self, job: dict, *, source_paths: list[str], base_path: str) -> dict:
        ...
        for idx, source_path in enumerate(source_paths, start=1):
            ...
            print(f"[TRACE] Fetching meta for {source_path}")
            file_meta = await self._file_manager.get_file_meta(source_path)
            print(f"[TRACE] Meta fetched for {source_path}")
            if file_meta:
                file_entries.append({
                    "kind": "file",
            ...
            tree = await self._file_manager._db.list_directory_tree(source_path)
    ...
    async def _materialize_collected_entries(self, job: dict, archive_manifest: dict, staging_root: Path) -> int:
        ...
        async def _download_one(entry: dict) -> None:
            async with semaphore:
                ...
                self._touch_job(
                    ...
                    current_file=file_name,
                    unit="items",
                    detail=f"Materializando {file_name}",
                )
        ...
        self._touch_job(
            ...
            unit="items",
            detail="Materialização para fallback staged",
        )
    ...
    async def _materialize_file(self, virtual_path: str, file_meta: dict, target_path: Path) -> None:
    ...
    def _extract_archive_to_directory(self, archive_path: Path, extract_root: Path, archive_format: str, password: str, job: dict) -> None:
        if archive_format == "zip":
    ...
    def _extract_zip(self, archive_path: Path, extract_root: Path, password: str, job: dict) -> None:
    ...
    def _extract_tar(self, archive_path: Path, extract_root: Path, job: dict) -> None:
    ...
    def _extract_rar(self, archive_path: Path, extract_root: Path, password: str, job: dict) -> None:
        if rarfile is None:
    ...
    def _extract_7z(self, archive_path: Path, extract_root: Path, password: str, job: dict) -> None:
        if py7zr is None:
            raise ArchiveCapabilityError("py7zr não está instalado.")
        ...
        self._touch_job(job, stage="extracting", current=total_entries, total=total_entries, message="Extraindo 7Z...")

    async def _upload_extracted_tree(self, job: dict, local_root: Path, destination_root: str, overwrite_mode: str) -> int:
        ...
        for directory in dirs:
            ...
            step += 1
            self._touch_job(job, stage="uploading", current=step, total=total_steps, message=f"Criando pasta: {directory.name}", current_file=directory.name)
        ...
        for file_path in files:
            ...
            if not final_path:
                ...
                step += 1
                self._touch_job(job, stage="uploading", current=step, total=total_steps, message=f"Conflito ignorado: {file_name}", current_file=file_name)
                continue
            self._touch_job(job, stage="uploading", current=step, total=total_steps, message=f"Enviando: {file_name} ({uploaded_files + 1}/{len(files)})", current_file=file_name)
            await self._file_manager.upload_file(file_path, final_path)
            uploaded_files += 1
            step += 1
            self._touch_job(job, stage="uploading", current=step, total=total_steps, message=f"Enviado: {file_name} ({uploaded_files}/{len(files)})", current_file=file_name)
