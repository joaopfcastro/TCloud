...
class TelegramManager:
    ...
    async def delete_files(self, message_ids: list[int]) -> bool:
        ...
        try:
            await client.delete_messages(Config.CHAT_ID, normalized_ids)
            logger.info(f"🗑️ Deleted {len(normalized_ids)} messages")
            return True
        except Exception as e:
            logger.warning(
                "⚠️ Batch delete failed for %d message(s), retrying individually: %s",
                len(normalized_ids),
                e,
            )
            ...
            for message_id in normalized_ids:
                try:
                    ...
                    logger.warning(
                        "⚠️ Skipping undeletable Telegram message %s: %s",
                        message_id,
                        single_error,
                    )
            ...
            if failed_ids:
                logger.warning(
                    "⚠️ Deleted %d/%d messages; %d could not be removed",
                    deleted_count,
                    len(normalized_ids),
                    len(failed_ids),
                )
            else:
                logger.info(f"🗑️ Deleted {deleted_count} messages after individual retry")
            return deleted_count == len(normalized_ids)
