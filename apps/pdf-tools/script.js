...
function ensurePdfRuntimeCompatibility() {
  ...
  if (
    ...
    ReadableStream.prototype[Symbol.asyncIterator] = async function* readableStreamAsyncIterator() {
...
function createSessionElements(tab) {
  ...
  annotationLayer.setAttribute("aria-label", "Anotacoes e links da pagina");
  ...
  const continuousLayer = document.createElement("div");
  continuousLayer.className = "continuous-layer";
  continuousLayer.setAttribute("aria-label", "Paginas em rolagem continua");

  pageLayer.append(canvas, textLayer, annotationLayer, searchHighlightLayer);
  spreadLayer.append(pageLayer, secondaryPageLayer);
  view.append(loading, skeleton, spreadLayer, continuousLayer);
...
function setSessionRenderElements(session, elements) {
  if (!session || !elements) return;
  session.elements.pageLayer = elements.pageLayer;
  session.elements.canvas = elements.canvas;
  session.elements.textLayer = elements.textLayer;
  session.elements.annotationLayer = elements.annotationLayer;
  session.elements.searchHighlightLayer = elements.searchHighlightLayer;
  if (session.documentKey === app.activeKey) syncLegacyDocumentState(session);
}
...
function normalizeRecentPdf(item) {
  ...
  return {
    ...
    document_key: String(item?.document_key || ""),
    opened_at: String(item?.opened_at || item?.updated_at || item?.modified_at || new Date().toISOString()),
  };
}

function recentPdfTime(item) {
  const value = Date.parse(item?.opened_at || "");
  return Number.isFinite(value) ? value : 0;
}

function mergeRecentPdfs(items) {
  const seen = new Set();
  return (items || [])
    .map(normalizeRecentPdf)
    .filter(Boolean)
    .sort((a, b) => recentPdfTime(b) - recentPdfTime(a))
    .filter((item) => {
...
async function loadRecentPdfs() {
  ...
  try {
    const data = await runtime("pdf.getRecentPdfs", { app_id: "pdf-tools" });
    app.recentPdfs = mergeRecentPdfs(data.recent_pdfs || []);
  } catch (error) {
    console.warn("Nao foi possivel carregar historico em nuvem de PDFs abertos", error);
    app.recentPdfs = mergeRecentPdfs(app.tabs);
  }
  renderRecentPdfs();
  publishState();
...
async function recordRecentPdfOpen(item) {
  const optimistic = mergeRecentPdfs([
    {
      ...item,
  ...
  app.recentPdfs = optimistic;
  renderRecentPdfs();
  publishState();
  try {
    const data = await runtime("pdf.recordRecentPdf", {
      app_id: "pdf-tools",
      document_key: item.document_key || "",
      path: item.path || "",
      name: item.name || "",
    });
    app.recentPdfs = mergeRecentPdfs(data.recent_pdfs || optimistic);
    renderRecentPdfs();
    publishState();
  } catch (error) {
    console.warn("Nao foi possivel salvar historico em nuvem de PDFs abertos", error);
  }
}
...
function renderRecentPdfs() {
  ...
  app.recentPdfs.forEach((item) => {
    ...
    button.querySelector(".recent-pdf-meta").textContent = "Aberto no PDF Tools";
...
async function openPdf(path, name = "", options = {}) {
  setSync("Abrindo");
  ...
  tab.updated_at = openedAt;
  tab.opened_at = tab.opened_at || openedAt;
  tab.name = tab.name || name || info.name || path.split("/").pop() || "PDF";
  recordRecentPdfOpen({
    document_key: documentKey,
    path: tab.path || info.path || path,
    name: tab.name,
    opened_at: openedAt,
  });
  app.activeKey = documentKey;
  ...
  const loadPromise = ensureSessionLoaded(session);
  if (app.activeKey === documentKey) {
    try {
      options.onSelectionReady?.({ tab, session, documentKey });
    } catch (error) {
      console.warn("Falha ao executar callback de selecao do PDF", error);
    }
  }
  await loadPromise;
  if (app.activeKey === documentKey) {
...
async function ensureSessionLoaded(session) {
  ...
  session.loadPromise = (async () => {
    try {
      ...
      hydratePageRotations(session, savedState.page_rotations);
...
function updateToolbar() {
  ...
  updateFitToggleUi(getActiveSession());
...
function nextFitMode(session = getActiveSession()) {
  return session?.fitMode === "width" ? "page" : "width";
}

function fitActionState(session = getActiveSession()) {
  const nextMode = nextFitMode(session);
  if (nextMode === "page") {
    return {
      mode: session?.fitMode || "page",
      nextMode,
      command: "fitPage",
      label: "Página inteira",
      shortLabel: "Página",
      title: "Ver página inteira",
...
function updateFitToggleUi(session = getActiveSession()) {
  if (!els.fitToggle) return;
  const state = fitActionState(session);
  els.fitToggle.title = state.title;
  els.fitToggle.setAttribute("aria-label", state.title);
  els.fitToggle.dataset.nextFitMode = state.nextMode;
  const icon = els.fitToggle.querySelector("i");
  if (icon) icon.className = state.icon;
  const label = els.fitToggle.querySelector("span");
  if (label) label.textContent = state.shortLabel;
}
...
function normalizeRotation(value) {
  ...
  return ((Math.round(parsed / 90) * 90) % 360 + 360) % 360;
}

function normalizePageRotations(rawRotations) {
  const source = rawRotations instanceof Map
    ? Object.fromEntries(rawRotations.entries())
    : (rawRotations && typeof rawRotations === "object" ? rawRotations : {});
  const normalized = {};
  Object.entries(source).forEach(([rawPage, rawRotation]) => {
    const pageNumber = Math.max(1, Number.parseInt(rawPage, 10) || 0);
    const rotation = normalizeRotation(rawRotation);
    if (pageNumber && rotation) normalized[String(pageNumber)] = rotation;
  });
  return normalized;
}

function serializePageRotations(session = getActiveSession()) {
  return normalizePageRotations(session?.pageRotations);
}

function hydratePageRotations(session, rawRotations) {
  if (!session) return;
  session.pageRotations = new Map(
    Object.entries(normalizePageRotations(rawRotations)).map(([page, rotation]) => [Number(page), rotation])
  );
}

function pageRotationSignature(session = getActiveSession()) {
  const rotations = serializePageRotations(session);
  return Object.keys(rotations)
    .sort((a, b) => Number(a) - Number(b))
    .map((page) => `${page}:${rotations[page]}`)
    .join("|");
}

function getPageRotation(session, pageNumber) {
  if (!session?.pageRotations) return 0;
  return normalizeRotation(session.pageRotations.get(Number(pageNumber)) || 0);
}

function setPageRotation(session, pageNumber, rotation) {
  if (!session) return 0;
  if (!session.pageRotations) session.pageRotations = new Map();
  const normalized = normalizeRotation(rotation);
  const page = Math.max(1, Number.parseInt(pageNumber, 10) || 1);
  if (normalized) {
    session.pageRotations.set(page, normalized);
  } else {
    session.pageRotations.delete(page);
  }
  return normalized;
}

function getViewportRotation(session, pageNumber, page) {
  return normalizeRotation((page?.rotate || 0) + getPageRotation(session, pageNumber));
}

function getPageViewport(page, session, pageNumber, scale) {
  return page.getViewport({
    scale,
    rotation: getViewportRotation(session, pageNumber, page),
  });
}
...
function debugLinksEnabled() {
  try {
    return localStorage.getItem("pdf-tools.debugLinks") === "1";
  } catch (error) {
    return false;
  }
}

function debugPdfLink(event, payload = {}) {
  if (!debugLinksEnabled()) return;
  console.info(`[PDF Tools links] ${event}`, payload);
}

function clampPageNumber(pageNumber, session = getActiveSession()) {
  const total = Math.max(1, Number(session?.totalPages || app.totalPages || 1));
  const parsed = Number(pageNumber || 1);
  if (!Number.isFinite(parsed)) return 1;
  return Math.min(total, Math.max(1, Math.round(parsed)));
}

function isPdfRef(value) {
  return value && typeof value === "object" && Number.isInteger(value.num) && Number.isInteger(value.gen);
}

async function resolvePdfDestinationPage(session, destination) {
  if (!session?.pdf || destination === null || destination === undefined) return null;
  let explicitDest = destination;
  if (typeof destination === "string") {
    explicitDest = await session.pdf.getDestination(destination);
  }
  if (!Array.isArray(explicitDest) || explicitDest.length < 1) {
    debugPdfLink("dest:invalid", { destination, explicitDest });
    return null;
  }
  const pageRef = explicitDest[0];
  if (Number.isInteger(pageRef)) {
    return clampPageNumber(pageRef + 1, session);
  }
  if (isPdfRef(pageRef)) {
    const cached = session.pdf.cachedPageNumber?.(pageRef);
    if (Number.isInteger(cached)) return clampPageNumber(cached, session);
    const pageIndex = await session.pdf.getPageIndex(pageRef);
    return clampPageNumber(pageIndex + 1, session);
  }
  debugPdfLink("dest:unsupported-page-ref", { destination, explicitDest, pageRef });
  return null;
}

function resolvePdfDestinationPageSync(session, destination) {
  if (!session?.pdf || !Array.isArray(destination) || destination.length < 1) return null;
  const pageRef = destination[0];
  if (Number.isInteger(pageRef)) return clampPageNumber(pageRef + 1, session);
  if (isPdfRef(pageRef)) {
    const cached = session.pdf.cachedPageNumber?.(pageRef);
    if (Number.isInteger(cached)) return clampPageNumber(cached, session);
  }
...
function createPdfToolsLinkService(session) {
  return {
    eventBus: null,
    addLinkAttributes(link, url, newWindow = false) {
      ...
      try {
        ...
        debugPdfLink("external-url:invalid", { href });
        return;
      }
      if (!["http:", "https:", "mailto:", "tel:"].includes(parsedUrl.protocol)) {
        debugPdfLink("external-url:blocked", { href, protocol: parsedUrl.protocol });
        return;
      }
      link.href = parsedUrl.href;
      link.target = "_blank";
      link.rel = "noopener noreferrer";
      link.title = link.title || parsedUrl.href;
    },
    getDestinationHash(destination) {
      const pageNumber = resolvePdfDestinationPageSync(session, destination);
      return pageNumber ? `#page=${pageNumber}` : "#";
    },
    getAnchorUrl(anchor = "") {
      return anchor ? `#${String(anchor).replace(/^#/, "")}` : "#";
    },
    goToDestination(destination) {
      resolvePdfDestinationPage(session, destination)
        .then((pageNumber) => {
          if (!pageNumber) return;
          debugPdfLink("dest:navigate", { destination, pageNumber, documentKey: session.documentKey });
          return goToPage(pageNumber, session);
        })
        .catch((error) => {
          console.warn("Falha ao navegar pelo link do PDF", error);
          debugPdfLink("dest:error", { destination, message: error?.message || String(error) });
        });
    },
    executeNamedAction(action) {
      const step = pageNavigationStep();
      const current = session?.page || app.page || 1;
      const actions = {
        FirstPage: 1,
        LastPage: session?.totalPages || app.totalPages || current,
        NextPage: current + step,
        PrevPage: current - step,
      };
      if (Object.prototype.hasOwnProperty.call(actions, action)) {
        goToPage(actions[action], session).catch(showError);
      } else {
        debugPdfLink("named-action:unsupported", { action });
      }
    },
    executeSetOCGState(action) {
      debugPdfLink("ocg:unsupported", { action });
    },
  };
}
...
function closeFindPanel(options = {}) {
  ...
  updateFindUi(session);
  publishState();
  if (options.focusStage !== false) els.stage?.focus({ preventScroll: true });
}

function toggleFindPanel(options = {}) {
  const session = getActiveSession();
  if (!session?.search) return;
  if (session.search.open) {
    closeFindPanel({ focusStage: false });
    return;
  }
  openFindPanel(options);
}
...
function publishState() {
  const session = getActiveSession();
  const search = session?.search || null;
  const fitState = fitActionState(session);
  window.parent.postMessage(
    {
      ...
      zoom: app.zoom || 1,
      fit_mode: session?.fitMode || app.fitMode || "page",
      fit_next_command: fitState.command,
      fit_next_label: fitState.label,
      fit_next_icon: fitState.icon,
      page_rotation: getPageRotation(session, session?.page || app.page || 1),
      page_rotations: serializePageRotations(session),
      sync: els.sync?.textContent || "Pronto",
      text_status: app.textStatus || "idle",
      text_status_reason: app.textStatusReason || "",
      active_document_key: app.activeKey || "",
      active_document_path: activeTab()?.path || "",
      active_document_name: activeTab()?.name || "",
...
function isAnnotationRenderCurrent(session, generation, record = null) {
  if (!session || session.destroyed) return false;
  if (record) return generation === session.continuous.generation && record.generation === generation;
  return generation === session.renderGeneration;
}

async function renderAnnotationLayer(session, page, viewport, generation, record = null) {
  const annotationLayerElement = record?.annotationLayer || session?.elements.annotationLayer;
  if (!session?.pdf || !annotationLayerElement) return;
  annotationLayerElement.replaceChildren();
  annotationLayerElement.style.setProperty("--total-scale-factor", String(viewport.scale || session.zoom || 1));
  ...
  try {
    ...
    if (!annotations.length || !isAnnotationRenderCurrent(session, generation, record)) return;
    const linkService = createPdfToolsLinkService(session);
    const annotationLayer = new app.pdfjs.AnnotationLayer({
      div: annotationLayerElement,
      page,
      viewport,
      linkService,
      annotationStorage: session.pdf.annotationStorage,
    });
    ...
    if (!isAnnotationRenderCurrent(session, generation, record)) {
      annotationLayerElement.replaceChildren();
      return;
    }
    debugPdfLink("render:complete", {
      documentKey: session.documentKey,
      page: Number(annotationLayerElement.dataset.page || session.page || 0),
      annotations: annotations.length,
      links: annotationLayerElement.querySelectorAll("a").length,
    });
  } catch (error) {
    if (isAnnotationRenderCurrent(session, generation, record)) {
      annotationLayerElement.replaceChildren();
    }
    console.warn("Falha ao renderizar links e anotacoes do PDF", error);
  }
}
...
function applyPageGeometry(session, viewport, outputScale, pageNumber, generation) {
  ...
  const { canvas, pageLayer, textLayer, annotationLayer, searchHighlightLayer } = session.elements;
  ...
  if (annotationLayer) {
    annotationLayer.style.width = cssWidth;
    annotationLayer.style.height = cssHeight;
    annotationLayer.style.setProperty("--total-scale-factor", String(viewport.scale || session.zoom || 1));
    annotationLayer.style.setProperty("--scale-round-x", "1px");
    annotationLayer.style.setProperty("--scale-round-y", "1px");
    annotationLayer.dataset.page = String(pageNumber);
    annotationLayer.dataset.generation = String(generation);
    annotationLayer.setAttribute("data-main-rotation", String(viewport.rotation || 0));
...
async function renderSecondarySpreadPage(session, pageNumber, generation) {
  ...
  try {
    ...
    const viewport = getPageViewport(page, session, secondaryNumber, session.zoom);
...
function shouldUseContinuousMode() {
  return app.preferences.scrollMode === "continuous" && !app.presentationActive;
}
...
async function renderPage(sessionOrPageNumber, maybePageNumber) {
  ...
  if (shouldUseContinuousMode()) {
    await renderContinuousDocument(session, pageNumber || session?.page || 1);
    return;
  }
  await renderPagedPage(session, pageNumber);
}

async function renderPagedPage(session, pageNumber) {
  if (!session?.pdf) return;
  teardownContinuousView(session, { keepNodes: true });
  setSessionRenderElements(session, session.elements.pagedElements);
  session.elements.view.classList.remove("continuous-view");
  session.elements.view.classList.add("paged-view");
  ...
  const viewport = getPageViewport(page, session, pageNumber, session.zoom);
  const outputScale = Math.max(1, window.devicePixelRatio || 1);
  ...
  if (generation !== session.renderGeneration || session.destroyed) return;
  await renderAnnotationLayer(session, page, viewport, generation);
  if (generation !== session.renderGeneration || session.destroyed) return;
  renderSearchHighlights(session);
...
function teardownContinuousView(session, options = {}) {
  const state = session?.continuous;
  if (!state) return;
  state.generation += 1;
  if (state.raf) {
  ...
  if (state.syncHoldTimer) {
    clearTimeout(state.syncHoldTimer);
    state.syncHoldTimer = 0;
  }
  ...
  state.pageNodes.forEach((record) => {
    if (record.renderTask) {
      try {
        record.renderTask.cancel();
      } catch (error) {
        console.warn("Falha ao cancelar renderizacao continua", error);
      }
    }
    if (record.textLayerTask?.cancel) {
      try {
        record.textLayerTask.cancel();
      } catch (error) {
        console.warn("Falha ao cancelar texto da rolagem continua", error);
      }
    }
  });
  if (!options.keepNodes) {
    session.elements.continuousLayer.replaceChildren();
...
function createContinuousPageNode(session, pageNumber, estimate) {
  const shell = document.createElement("div");
  shell.className = "continuous-page-shell";
  shell.dataset.page = String(pageNumber);
  shell.style.minHeight = `${Math.max(240, estimate.height)}px`;

  const pageLayer = document.createElement("div");
  pageLayer.className = "page-layer continuous-page-layer";
  pageLayer.style.width = `${Math.max(160, estimate.width)}px`;
  pageLayer.style.height = `${Math.max(240, estimate.height)}px`;
  pageLayer.dataset.page = String(pageNumber);

  const canvas = document.createElement("canvas");
  const textLayer = document.createElement("div");
  textLayer.className = "textLayer";
  textLayer.setAttribute("aria-label", `Texto da pagina ${pageNumber}`);
  const annotationLayer = document.createElement("div");
  annotationLayer.className = "annotationLayer";
  annotationLayer.setAttribute("aria-label", `Anotacoes e links da pagina ${pageNumber}`);
  const searchHighlightLayer = document.createElement("div");
  searchHighlightLayer.className = "search-highlight-layer";
  searchHighlightLayer.setAttribute("aria-hidden", "true");

  pageLayer.append(canvas, textLayer, annotationLayer, searchHighlightLayer);
  shell.appendChild(pageLayer);
...
async function renderContinuousDocument(session, targetPage = session?.page || 1) {
  if (!session?.pdf) return;
  setSessionRenderElements(session, session.elements.pagedElements);
  session.elements.view.classList.add("continuous-view");
  session.elements.view.classList.remove("paged-view");
  session.elements.spreadLayer.classList.remove("two-page");
  session.elements.view.classList.remove("two-page-view");
  hideSecondaryPage(session);
  ...
  if (!session.hasRender) {
    session.elements.view.classList.add("loading", "rendering-page");
    session.elements.view.classList.remove("loading-document", "load-error");
    setDocumentLoadingText(session, "Montando paginas");
  }

  const clampedTarget = Math.min(session.totalPages, Math.max(1, Number(targetPage || 1)));
  const firstPage = await session.pdf.getPage(clampedTarget);
  if (generation !== session.renderGeneration || session.destroyed) return;
  const firstViewport = getPageViewport(firstPage, session, clampedTarget, session.zoom);
  const estimate = { width: firstViewport.width, height: firstViewport.height };
  const rotationSignature = pageRotationSignature(session);
  const needsRebuild = session.continuous.zoom !== session.zoom
    || session.continuous.totalPages !== session.totalPages
    || session.continuous.pageNodes.size !== session.totalPages
    || session.continuous.rotationSignature !== rotationSignature;

  if (needsRebuild) {
    teardownContinuousView(session, { keepNodes: false });
    session.elements.view.classList.add("continuous-view");
    session.continuous.zoom = session.zoom;
    session.continuous.totalPages = session.totalPages;
    session.continuous.rotationSignature = rotationSignature;
    const fragment = document.createDocumentFragment();
    for (let page = 1; page <= session.totalPages; page += 1) {
      const record = createContinuousPageNode(session, page, estimate);
      session.continuous.pageNodes.set(page, record);
      fragment.appendChild(record.shell);
    }
    session.elements.continuousLayer.appendChild(fragment);
  }
  session.continuous.generation += 1;
  const continuousGeneration = session.continuous.generation;

  session.page = clampedTarget;
  session.continuous.scrollTarget = clampedTarget;
  holdContinuousPageSync(session);
  syncLegacyDocumentState(session);
  updateToolbar();

  setupContinuousObserver(session, continuousGeneration);
  const targetRecord = session.continuous.pageNodes.get(clampedTarget);
  if (targetRecord) {
    targetRecord.shell.scrollIntoView({ block: "start", inline: "nearest" });
    await renderContinuousPage(session, clampedTarget, continuousGeneration, firstPage);
    if (targetRecord.rendered) {
      setSessionRenderElements(session, targetRecord);
      renderSearchHighlights(session);
    }
  }
  syncLegacyDocumentState(session);
  ...
  session.elements.view.classList.add("has-render");
  session.elements.view.classList.remove("loading", "loading-document", "rendering-page", "load-error");
  markPdfPerf("continuous:ready", { documentKey: session.documentKey, page: clampedTarget, pages: session.totalPages });
  scheduleSave("rolagem continua");
}

function setupContinuousObserver(session, generation) {
  if (session.continuous.observer) {
    session.continuous.observer.disconnect();
    session.continuous.observer = null;
  }
  session.continuous.observer = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      if (!entry.isIntersecting) return;
      const pageNumber = Number(entry.target.dataset.page || 0);
      if (!pageNumber) return;
      renderContinuousPage(session, pageNumber, generation).catch(showError);
    });
  }, { root: els.stage, rootMargin: PDF_CONTINUOUS_RENDER_MARGIN, threshold: 0.01 });
  session.continuous.pageNodes.forEach((record) => {
    session.continuous.observer.observe(record.shell);
  });
}

async function renderContinuousPage(session, pageNumber, generation, knownPage = null) {
  const record = session?.continuous?.pageNodes?.get(pageNumber);
  if (!session?.pdf || !record || record.rendered || record.renderTask || session.destroyed) return;
  if (generation !== session.continuous.generation) return;
  record.generation = generation;
  const page = knownPage || await session.pdf.getPage(pageNumber);
  if (generation !== session.continuous.generation || session.destroyed) return;
  const viewport = getPageViewport(page, session, pageNumber, session.zoom);
  const outputScale = Math.max(1, window.devicePixelRatio || 1);
  ...
  record.pageLayer.style.width = `${viewport.width}px`;
  record.pageLayer.style.height = `${viewport.height}px`;
  record.shell.style.minHeight = `${viewport.height}px`;
  record.pageLayer.dataset.zoom = String(session.zoom || viewport.scale || 1);
  record.pageLayer.dataset.outputScale = String(outputScale);
  record.textLayer.style.width = `${viewport.width}px`;
  record.textLayer.style.height = `${viewport.height}px`;
  record.textLayer.style.setProperty("--total-scale-factor", String(viewport.scale || session.zoom || 1));
  record.textLayer.style.setProperty("--scale-round-x", "1px");
  record.textLayer.style.setProperty("--scale-round-y", "1px");
  record.textLayer.dataset.page = String(pageNumber);
  record.textLayer.dataset.generation = String(generation);
  record.textLayer.setAttribute("data-main-rotation", String(viewport.rotation || 0));
  record.annotationLayer.style.width = `${viewport.width}px`;
  record.annotationLayer.style.height = `${viewport.height}px`;
  record.annotationLayer.style.setProperty("--total-scale-factor", String(viewport.scale || session.zoom || 1));
  record.annotationLayer.style.setProperty("--scale-round-x", "1px");
  record.annotationLayer.style.setProperty("--scale-round-y", "1px");
  record.annotationLayer.dataset.page = String(pageNumber);
  record.annotationLayer.dataset.generation = String(generation);
  record.annotationLayer.setAttribute("data-main-rotation", String(viewport.rotation || 0));
  record.annotationLayer.replaceChildren();
  record.searchHighlightLayer.style.width = `${viewport.width}px`;
  record.searchHighlightLayer.style.height = `${viewport.height}px`;

  const context = record.canvas.getContext("2d", { alpha: false });
  const transform = outputScale !== 1 ? [outputScale, 0, 0, outputScale, 0, 0] : null;
  record.renderTask = page.render({ canvasContext: context, viewport, transform });
  try {
  ...
  record.shell.classList.add("rendered");
  await renderContinuousTextLayer(session, record, page, viewport, generation);
  await renderAnnotationLayer(session, page, viewport, generation, record);
  if (pageNumber === session.page) {
    setSessionRenderElements(session, record);
    renderSearchHighlights(session);
  }
}

async function renderContinuousTextLayer(session, record, page, viewport, generation) {
  ...
  record.textLayerTask = textLayer;
  try {
    await textLayer.render();
  } catch (error) {
    if (error?.name !== "AbortException") {
      console.warn("Falha ao renderizar texto na rolagem continua", error);
    }
  } finally {
    if (record.textLayerTask === textLayer) record.textLayerTask = null;
  }
}

function updateCurrentPageFromScroll(session = getActiveSession()) {
  if (!session?.pdf || !shouldUseContinuousMode()) return;
  if (Date.now() < Number(session.continuous.syncHoldUntil || 0)) return;
  const stageRect = els.stage.getBoundingClientRect();
  const anchorY = stageRect.top + Math.min(stageRect.height * 0.35, 260);
  let bestPage = session.page;
  let bestDistance = Number.POSITIVE_INFINITY;
  session.continuous.pageNodes.forEach((record, pageNumber) => {
    const rect = record.shell.getBoundingClientRect();
    const distance = Math.abs(rect.top - anchorY);
    if (rect.bottom >= stageRect.top && rect.top <= stageRect.bottom && distance < bestDistance) {
      bestDistance = distance;
      bestPage = pageNumber;
    }
  });
  if (bestPage && bestPage !== session.page) {
    session.page = bestPage;
    const record = session.continuous.pageNodes.get(bestPage);
    if (record?.rendered) {
      setSessionRenderElements(session, record);
      renderSearchHighlights(session);
    }
...
function holdContinuousPageSync(session = getActiveSession(), duration = 180) {
  if (!session?.continuous) return;
  session.continuous.syncHoldUntil = Date.now() + duration;
  if (session.continuous.syncHoldTimer) {
    clearTimeout(session.continuous.syncHoldTimer);
  }
  session.continuous.syncHoldTimer = setTimeout(() => {
    session.continuous.syncHoldTimer = 0;
    queueContinuousPageSync();
  }, duration + 20);
}

function queueContinuousPageSync() {
  const session = getActiveSession();
  if (!session?.pdf || !shouldUseContinuousMode() || session.continuous.raf) return;
  session.continuous.raf = requestAnimationFrame(() => {
    session.continuous.raf = 0;
    updateCurrentPageFromScroll(session);
  });
}
...
async function computeFitPageZoom(sessionOrPageNumber = getActiveSession(), maybePageNumber) {
  ...
  const page = await session.pdf.getPage(pageNumber);
  const viewport = getPageViewport(page, session, pageNumber, 1);
...
function invalidateThumb(session, pageNumber) {
  if (!session?.elements?.thumbsList) return;
  const page = Number(pageNumber || 0);
  if (!page) return;
  session.renderedThumbs.delete(page);
  const node = session.elements.thumbsList.querySelector(`.thumb[data-page="${page}"]`);
  if (!node) return;
  ...
  renderThumb(session, page, node).catch(showError);
}

async function rotateCurrentPageRight(session = getActiveSession()) {
  if (!session?.pdf) return;
  const pageNumber = Math.min(session.totalPages || 1, Math.max(1, Number(session.page || 1)));
  setPageRotation(session, pageNumber, getPageRotation(session, pageNumber) + 90);
  clearSearchHighlights(session);
  invalidateThumb(session, pageNumber);
  if (shouldUseContinuousMode()) {
    teardownContinuousView(session, { keepNodes: false });
  }
...
function createThumbButton(page, session) {
  ...
  button.innerHTML = `
    ...
    <span class="thumb-label">Pagina ${page}</span>
  `;
...
async function renderThumb(session, pageNumber, node) {
  ...
  try {
    const page = await session.pdf.getPage(pageNumber);
    const canvas = node.querySelector("canvas");
    const preview = node.querySelector(".thumb-preview");
    const dpr = window.devicePixelRatio || 1;
    const cssViewport = getPageViewport(page, session, pageNumber, 0.2);
    const viewport = getPageViewport(page, session, pageNumber, 0.2 * dpr);
    ...
    if (preview) {
      preview.style.aspectRatio = `${cssWidth} / ${cssHeight}`;
    }
    await page.render({
...
async function goToPage(pageNumber, session = getActiveSession()) {
  ...
  markActiveThumb(session);
  publishState();
  if (shouldUseContinuousMode()) holdContinuousPageSync(session);
  if (target === previousPage) {
    if (shouldUseContinuousMode()) {
      const record = session.continuous.pageNodes.get(target);
      if (record) record.shell.scrollIntoView({ block: "start", inline: "nearest" });
    }
    return;
  }
  if (app.presentationActive || session.fitMode === "page") {
...
function handlePageKeyboard(event) {
  ...
  if (shouldUseContinuousMode() && session?.pdf) {
    if (key === "ArrowDown" || key === "PageDown" || key === " ") {
      event.preventDefault();
      els.stage.scrollBy({ top: Math.max(120, els.stage.clientHeight * 0.82), behavior: "smooth" });
      return;
    }
...
function wireEvents() {
  ...
  if (els.fitToggle) els.fitToggle.onclick = () => toggleFitMode().catch(showError);
  if (els.rotateRight) els.rotateRight.onclick = () => rotateCurrentPageRight().catch(showError);
  ...
  els.stage.addEventListener("scroll", queueContinuousPageSync, { passive: true });
...
async function handleShellCommand(command, payload = {}) {
  ...
  if (command === "toggleFitMode") return toggleFitMode();
  if (command === "rotateRight") return rotateCurrentPageRight();
  if (command === "zoomOut") {
    const session = getActiveSession();
